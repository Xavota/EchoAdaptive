...
void
SoundTrack::writeSoundData(float* data, int i)
{
  ...
  if (realPosition + sound->numChannels - 1 >= sound->count) return;
...
void
SoundTrack::getSoundData(float& ldata, float& rdata)
{
  // If there is no sound assigned to the channel do nothing
  if (!sound || !sound->data || sound->numChannels == 0) return;
  // We need to write "count" samples to the "data" array
  ...
  // Read value from the sound data at the current position
  if (sound->numChannels == 1) {
    float value = static_cast<float>(sound->data[realPosition] * volume);
    // Write value to both the left and right channels
    ldata  = static_cast<float>(value * leftGain);
    rdata  = static_cast<float>(value * rightGain);
  }
  else if (sound->numChannels > 1) {
    // Write value to both the left and right channels
    ldata = static_cast<float>(sound->data[realPosition] * volume * leftGain);
    rdata = static_cast<float>(sound->data[realPosition + 1] * volume * rightGain);
  }
  else {
    ldata = 0.0f;
    rdata = 0.0f;
